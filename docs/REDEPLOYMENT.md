...
(see `deployment/environment_variables_and_secrets_info.md` for what goes where):
...
- **Destructive** (renamed/removed field, changed type): needs a migration
  ...
  a manual Postgres backup before deploying this kind of change (section 7.1),
  on top of the automatic pre-deploy backup.
...
you clear the flag (see `TestUser` in section 8).
...
- **Code**: re-run the workflow against an older commit:
  ...
  redeploys it.
- **Data**: restore from a pre-deploy backup in `backups/deploy/` if the bad
  deploy also corrupted data — follow section 7.3, which drops and recreates
  the database first (a plain restore into a live database fails on every
  object that already exists).

## 7. Postgres backups by hand

Automatic dumps already exist on the VPS under `/opt/gyertek-velunk/backups/`:
`deploy/` (one per deploy, last 5) and `scheduled/` (daily 03:00 UTC, last 7).
You can also trigger the scheduled one on demand:

```bash
gh workflow run backup.yml --ref main
```

### 7.1 Dump prod right now
...
docker compose exec -T postgres sh -c 'pg_dump -U "$POSTGRES_USER" "$POSTGRES_DB"' \
  > backups/manual/backup_$(date +%Y%m%d%H%M%S).sql
```

Plain-SQL dump, no `--clean`, so restoring it expects an **empty** database
(see 7.3). Credentials come from the container's own env — nothing to type.

### 7.2 Copy a backup down to your machine

List what's there, then pull the one you want (run from a local PowerShell, not
inside the SSH session):

```bash
ssh user@VPS_IP "ls -1t /opt/gyertek-velunk/backups/scheduled /opt/gyertek-velunk/backups/deploy"
scp user@VPS_IP:/opt/gyertek-velunk/backups/scheduled/backup_20260917030000.sql .
```

Whole backup tree at once:

```bash
scp -r user@VPS_IP:/opt/gyertek-velunk/backups ./backups-from-vps
```

Don't pipe `ssh ... pg_dump` straight into a local `>` redirect in PowerShell —
Windows PowerShell writes UTF-16 and corrupts the `.sql`. Dump to a file on the
VPS (7.1) and `scp` it down.

### 7.3 Upload a backup and restore it onto prod

**Destructive** — the current prod database is dropped. Take a fresh dump (7.1)
first, and preferably copy it down (7.2) before doing this.

```bash
scp backup_20260917030000.sql user@VPS_IP:/opt/gyertek-velunk/backups/manual/
```

```bash
ssh user@VPS_IP
cd /opt/gyertek-velunk
docker compose stop strapi client                      # drop open connections
docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d postgres \
  -c "DROP DATABASE \"$POSTGRES_DB\" WITH (FORCE);" \
  -c "CREATE DATABASE \"$POSTGRES_DB\" OWNER \"$POSTGRES_USER\";"'
docker compose exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"' \
  < backups/manual/backup_20260917030000.sql
docker compose up -d
docker compose logs -f strapi                          # schema sync should be a no-op
```
