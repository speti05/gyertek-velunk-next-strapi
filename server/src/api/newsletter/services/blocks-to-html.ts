...
function renderInline(node: InlineNode): string {
  ...
  if (node.code) text = `<code style="background:#f4f4f4;padding:2px 6px;border-radius:3px;font-family:monospace;font-size:14px;">${text}</code>`;
  return text;
}
...
function renderBlock(block: BlockNode): string {
  ...
  switch (block.type) {
    ...
    case 'heading': {
      ...
      const content = children.map(renderInline).join('');
      const sizeMap: Record<number, string> = { 1: '28px', 2: '24px', 3: '20px', 4: '18px', 5: '16px', 6: '14px' };
