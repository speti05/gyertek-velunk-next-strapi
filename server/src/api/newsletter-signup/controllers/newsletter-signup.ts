...
export default factories.createCoreController(UID, ({ strapi }) => ({
  ...
  async unsubscribe(ctx) {
    // Reached by clicking a link in a newsletter, and a newsletter signup stores only an
    // e-mail address - the reader's language is unknown, so this page is default-locale.
    ...
    const fail = (message: string) => {
      ctx.type = "html";
      ctx.status = 400;
      ctx.body = unsubscribePage(texts, texts.UNSUBSCRIBE_ERROR_TITLE, message, false);
    };

    if (!email || !token) return fail(texts.UNSUBSCRIBE_INCOMPLETE_LINK);
    if (!verifyUnsubscribeToken(email, token)) return fail(texts.UNSUBSCRIBE_INVALID_LINK);
    ...
    ctx.type = "html";
    ctx.body = unsubscribePage(texts, texts.UNSUBSCRIBE_SUCCESS_TITLE, texts.UNSUBSCRIBE_SUCCESS_MESSAGE, true);
  },
}));
...
<body style="margin:0;padding:0;background:#F1E8D9;font-family:'Source Sans 3',Arial,sans-serif;display:flex;align-items:center;justify-content:center;min-height:100vh;">
  <div style="max-width:480px;margin:40px auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.12);text-align:center;">
    <div style="background:${color};padding:32px 40px;">
      <h1 style="font-family:'Luckiest Guy',cursive;color:#fff;margin:0;font-size:28px;letter-spacing:2px;font-weight:400;">${t.SITE_NAME}</h1>
    </div>
    <div style="padding:48px 40px;">
      ...
      <a href="${siteUrl}" style="background:${color};color:#fff;padding:12px 28px;border-radius:8px;text-decoration:none;font-size:16px;font-weight:600;">${t.GO_HOME_LABEL}</a>
    </div>
    <div style="background:#70634C;padding:20px 40px;text-align:center;">
      <p style="color:#F1E8D9;font-size:13px;margin:0;">&copy; ${new Date().getFullYear()} ${t.SITE_NAME}</p>
    </div>
  </div>
</body>
</html>`;
