/**
 * Hungarian half of the backend's language files - the source of truth for every
 * user-facing string the backend produces: e-mail subjects and bodies, the HTML pages
 * Strapi serves itself, and API error messages returned to the client.
 *
 * Mirrors the client's `client/src/i18n/texts-client.ts`. The English overrides live in
 * `texts-strapi-en.ts`, and `get-strapi-texts.ts` merges the two - never import from
 * this file directly in a template, go through `getStrapiTexts(locale)` so the reader's
 * language is honoured.
 *
 * Strings that embed runtime values are exported as functions. Markup stays in the
 * templates - a value interpolated into a sentence is passed in already wrapped
 * (e.g. `<strong>...</strong>`) so the whole sentence stays in one key.
 */
...
// E-mail header banner titles. A plain object rather than an enum: enum members cannot
// be overridden by a translation, and these have to follow the reader's language.
...
/** Slug of the Strapi page the contract is built from. Localized on the page type,
 *  so the lookup has to pick the slug of the locale it is fetching. */
export const TRAVEL_CONTRACT_SLUG = "utazasi-szerzodes";
...
/**
 * Subject lines to paste into the Strapi admin alongside the generated HTML. The subject
 * field is a separate input there, so it has to carry both languages too.
 */
export const SYSTEM_EMAIL_SUBJECT_BILINGUAL = {
  emailConfirmation: "Email cím megerősítése / Confirm your e-mail address",
  forgotPassword: "Elfelejtett jelszó / Password reset",
};
