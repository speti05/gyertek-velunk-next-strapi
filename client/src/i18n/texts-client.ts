/**
 * Hungarian half of the Next.js app's language files - the source of truth for every
 * user-facing string the frontend renders.
 *
 * The English overrides live in `texts-client-en.ts`, and `texts.ts` merges the two.
 * Components never import from this file directly, they go through `useTexts()` or
 * `getTexts(locale)` so the visitor's language is honoured. The one exception is a value
 * that is persisted or compared rather than displayed - see TourSignupDialog.
 *
 * The backend keeps its own pair, `server/src/i18n/texts-strapi*.ts`.
 */
...
// Pagination
export const PAGINATION_PAGE_LABEL = "Oldal";
export const PAGINATION_NAV_ARIA = "Lapozás";
...
// Language switcher
// Language names stay in their own language (endonyms), so they are identical in every
// dictionary and deliberately not overridden in texts-en.
export const LANGUAGE_SWITCHER_LABEL = "Nyelv";
...
export const PROFILE_PAYMENT_PENDING = "Befizetés folyamatban";
export const PROFILE_PAYMENT_DEPOSIT_PAID = "Előleg befizetve";
export const PROFILE_PAYMENT_PAID = "Befizetve";
export const PROFILE_PAYMENT_CANCELLED = "Lemondva";
...
// Strapi page slugs referenced from code. These are content, not route segments, so
// nothing translates them at render time - each locale holds its own slug, and the
// matching page has to exist in Strapi under exactly this slug.
// Links go through useLocalizedPath() to pick up the locale prefix.
export const TERMS_LINK = "/aszf";
export const PRIVACY_LINK = "/adatvedelem";
export const TRAVEL_CONTRACT_LINK = "/utazasi-szerzodes";

// Slugs of the Strapi pages the section routes load their blocks from.
export const REPORTS_PAGE_SLUG = "beszamolok";
