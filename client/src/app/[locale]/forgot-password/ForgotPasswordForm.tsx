...
function SubmitBtn() {
  const { LOADING_LABEL, AUTH_FORGOT_PASSWORD_TITLE } = useTexts();
...
function ForgotPasswordFormInner() {
  const { AUTH_BACK_TO_LOGIN, FORM_LABELS } = useTexts();
  ...
  return (
    <>
      ...
      <p className="auth-page__forgot-link">
        <CustomLink href={localizePath(Route.Login)} color="secondary">{AUTH_BACK_TO_LOGIN}</CustomLink>
      </p>
    </>
  );
}
